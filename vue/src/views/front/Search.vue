    ...
    <div style="display: flex; width: 70%; background-color: white; margin: 30px auto; border-radius: 20px">
      <div style="flex: 1; padding: 0 20px">
        ...
        <div style="margin: 20px 0">
          <el-row :gutter="20">
            <el-col :span="6" style="margin-bottom: 20px" v-for="item in courseData">
              <img @click="navTo('/front/courseDetail?id=' + item.id)" :src="item.img" alt="" style="width: 100%; height: 175px; border-radius: 10px; border: #cccccc 1px solid">
...
export default {
  ...
  methods: {
    ...
    loadGoods() {
      this.$request.get('/course/selectByName?name=' + this.name).then(res => {
        if (res.code === '200') {
